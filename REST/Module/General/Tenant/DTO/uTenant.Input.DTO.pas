...
type
  TTenantInputDTO = class(TBaseDTO)
  private
  ...
  public
    {$IFDEF APPREST}
    class function FromReq(AReq: THorseRequest): TTenantInputDTO;
    {$ENDIF}
...
{$IFDEF APPREST}
class function TTenantInputDTO.FromReq(AReq: THorseRequest): TTenantInputDTO;
begin
  if AReq.Body.Trim.IsEmpty then
    raise Exception.Create('Nenhum JSON informado!');

  Result             := TTenantInputDTO.FromJSON(AReq.Body);
