...
begin
  ...
  // Obter FilterDTO
  const LInput: SH<TPersonFilterDTO> = TPersonFilterDTO.FromReq(FReq);
...
begin
  ...
  // Procurar por ID
  const LOutput: SH<TPersonShowDTO> = FPersistence.Show(LID);
...
begin
  ...
  // Retorno
  const LOutput: SH<TPersonShowDTO> = LUseCaseResult.Right;
