...
procedure TSizeTest.Setup;
begin
  FPersistence := TSizePersistenceUseCase.Make(TRepositoryFactory.Make(TestConnection.Conn).Size);
  FDataFactory        := TSizeDataFactory.Make(FPersistence);
end;

procedure TSizeTest.should_update_by_id;
begin
...
procedure TSizeTest.should_find_by_id;
begin
...
procedure TSizeTest.should_delete_by_id;
begin
...
procedure TSizeTest.should_include;
begin
  // Inserir registro
  const LStored = FDataFactory.GenerateInsert;
...
procedure TSizeTest.should_list_records;
const
  EXPECTED_COUNT = 3;
...
begin
  ...
  begin
    const LStored: SH<TSizeShowDTO> = FDataFactory.GenerateInsert;
    LPks := LPks + LStored.Value.id.ToString + ', ';
  end;
  LPks := Copy(LPks, 1, Length(LPks)-2);

  // Listar registros
  const SizeFilterDTO: SH<TSizeFilterDTO> = TSizeFilterDTO.Create;
  SizeFilterDTO.Value.where_pk_in := LPks;
  const LIndexResult = FPersistence.Index(SizeFilterDTO);
...
initialization
  TDUnitX.RegisterTestFixture(TSizeTest);
end.
