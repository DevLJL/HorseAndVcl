...
type
  [TestFixture]
  TCostCenterTest = class
  private
    FPersistence: ICostCenterPersistenceUseCase;
    FDataFactory: ICostCenterDataFactory;
  public
    [Setup]
    procedure Setup;
...
procedure TCostCenterTest.Setup;
begin
  FPersistence := TCostCenterPersistenceUseCase.Make(TRepositoryFactory.Make(TestConnection.Conn).CostCenter);
  FDataFactory        := TCostCenterDataFactory.Make(FPersistence);
end;

procedure TCostCenterTest.should_update_by_id;
begin
...
procedure TCostCenterTest.should_find_by_id;
begin
...
procedure TCostCenterTest.should_delete_by_id;
begin
...
procedure TCostCenterTest.should_include;
begin
  // Inserir registro
  const LStored = FDataFactory.GenerateInsert;
...
procedure TCostCenterTest.should_list_records;
const
  EXPECTED_COUNT = 3;
...
begin
  ...
  begin
    const LStored: SH<TCostCenterShowDTO> = FDataFactory.GenerateInsert;
    LPks := LPks + LStored.Value.id.ToString + ', ';
  end;
  LPks := Copy(LPks, 1, Length(LPks)-2);

  // Listar registros
  const CostCenterFilterDTO: SH<TCostCenterFilterDTO> = TCostCenterFilterDTO.Create;
  CostCenterFilterDTO.Value.where_pk_in := LPks;
  const LIndexResult = FPersistence.Index(CostCenterFilterDTO);
...
initialization
  TDUnitX.RegisterTestFixture(TCostCenterTest);
end.
