...
begin
  // Obter FilterDTO
  const LInput: SH<TCostCenterFilterDTO> = TCostCenterFilterDTO.FromReq(FReq);
...
begin
  ...
  // Procurar por ID
  const LOutput: SH<TCostCenterShowDTO> = FPersistence.Show(LID);
...
begin
  ...
  // Retorno
  const LOutput: SH<TCostCenterShowDTO> = LUseCaseResult.Right;
