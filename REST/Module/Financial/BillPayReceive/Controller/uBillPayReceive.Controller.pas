...
begin
  // Obter FilterDTO
  const LInput: SH<TBillPayReceiveFilterDTO> = TBillPayReceiveFilterDTO.FromReq(FReq);
...
begin
  ...
  // Procurar por ID
  const LOutput: SH<TBillPayReceiveShowDTO> = FPersistence.Show(LID);
...
begin
  ...
  // Retorno
  const LOutput: SH<TBillPayReceiveShowDTO> = LUseCaseResult.Right;
