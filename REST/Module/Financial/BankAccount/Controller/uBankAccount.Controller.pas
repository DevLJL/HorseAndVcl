...
begin
  // Obter FilterDTO
  const LInput: SH<TBankAccountFilterDTO> = TBankAccountFilterDTO.FromReq(FReq);
...
begin
  ...
  // Procurar por ID
  const LOutput: SH<TBankAccountShowDTO> = FPersistence.Show(LID);
...
begin
  ...
  // Retorno
  const LOutput: SH<TBankAccountShowDTO> = LUseCaseResult.Right;
