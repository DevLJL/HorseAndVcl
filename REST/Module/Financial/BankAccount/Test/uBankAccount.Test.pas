...
type
  [TestFixture]
  TBankAccountTest = class
  private
    FPersistence: IBankAccountPersistenceUseCase;
    FDataFactory: IBankAccountDataFactory;
  public
    [Setup]
    procedure Setup;
...
begin
  ...
  FDataFactory        := TBankAccountDataFactory.Make(FPersistence);
end;

procedure TBankAccountTest.should_update_by_id;
begin
...
procedure TBankAccountTest.should_find_by_id;
begin
...
procedure TBankAccountTest.should_delete_by_id;
begin
...
procedure TBankAccountTest.should_include;
begin
  // Inserir registro
  const LStored = FDataFactory.GenerateInsert;
...
procedure TBankAccountTest.should_list_records;
const
  EXPECTED_COUNT = 3;
...
begin
  ...
  begin
    const LStored: SH<TBankAccountShowDTO> = FDataFactory.GenerateInsert;
    LPks := LPks + LStored.Value.id.ToString + ', ';
  end;
  LPks := Copy(LPks, 1, Length(LPks)-2);

  // Listar registros
  const BankAccountFilterDTO: SH<TBankAccountFilterDTO> = TBankAccountFilterDTO.Create;
  BankAccountFilterDTO.Value.where_pk_in := LPks;
  const LIndexResult = FPersistence.Index(BankAccountFilterDTO);
...
initialization
  TDUnitX.RegisterTestFixture(TBankAccountTest);
end.
