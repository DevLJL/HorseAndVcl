...
type
  ...
  function  PercentageOfValue(APercentual, AAmount: Double): Double;
  function  VarFloat(const V: Variant; ADefaultValue: Double = 0): Double;
  function  TheKeyIsBeingPressed(const Key: integer): boolean;
  procedure RoundPanel(AComponent: TWinControl);
  procedure FitFormToScreen(AForm: TForm);
  procedure LockControl(ABackgroundControl: TControl = nil);
  procedure UnlockControl(ABackgroundControl: TControl = nil);
  procedure OpenFile(APathFile: String);
  ...
  function  Space(N: integer): string; // Prencher espaços
  function  SpaceStr(AContent: String; ALength: integer; ASide: TSide): string; // Prencher espaços
  ...
  function  StrInt64(AValue: String; ADefaultValue: Int64 = 0): Int64;
  ...
  function  Iif(aCondition: Boolean; aResultTrue, aResultFalse: variant): variant; // Simula operador ternário
...
begin
  ...
  LBytes := TBase64Encoding.Base64.DecodeStringToBytes(ABase64);
...
begin
  LInStream := TFileStream.Create(AFilePath, fmOpenRead);
...
function PercentageOfValue(APercentual, AAmount: Double): Double;
begin
  Result := AAmount * (APercentual/100);
end;

function VarFloat(const V: Variant; ADefaultValue: Double): Double;
var
  FloatValue: Double;
begin
  ...
  begin
    ...
    raise Exception.Create('A variant não pode ser convertida em Float');
end;
...
begin
  if Assigned(ABackgroundControl) then
...
begin
  ...
  // Apagar ultima linha em branco se existir
  if sl[Pred(sl.Count)].Trim.IsEmpty then
...
function StrInt64(AValue: String; ADefaultValue: Int64): Int64;
begin
