...
begin
  ...
  case AData.Trim.IsEmpty of
    ...
    False: Begin
      lDataStrList := TStringList.Create;
      parseDelimited(lDataStrList, AData, DELIMITED_CHAR);
      for lI := 0 to Pred(lDataStrList.Value.Count) do
        if not lDataStrList.Value[lI].Trim.IsEmpty then
          lReturn.A['data'].Add(lDataStrList.Value[lI]);
    end;
  end;
