unit uChartOfAccount.Index.View;
...
type
  TChartOfAccountIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  ChartOfAccountIndexView: TChartOfAccountIndexView;
...
procedure TChartOfAccountIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Planos de Conta';
  pnlAppend.Width            := 0;
...
procedure TChartOfAccountIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TChartOfAccountIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TChartOfAccountShowDTO> = TChartOfAccountInputView.Handle(TEntityState.Store);
...
procedure TChartOfAccountIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TChartOfAccountService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TChartOfAccountIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TChartOfAccountShowDTO> = TChartOfAccountInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TChartOfAccountIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TChartOfAccountIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TChartOfAccountIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TChartOfAccountIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TChartOfAccountIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TChartOfAccountInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TChartOfAccountIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TChartOfAccountIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'chart_of_account.name';
...
procedure TChartOfAccountIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TChartOfAccountIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TChartOfAccountIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TChartOfAccountIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'chart_of_account.'+LSelectedColumn;
...
procedure TChartOfAccountIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TChartOfAccountFilterDTO> = TChartOfAccountFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TChartOfAccountService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TChartOfAccountIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TChartOfAccountIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TChartOfAccountIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TChartOfAccountIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TChartOfAccountIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TChartOfAccountIndexView> = TChartOfAccountIndexView.Create(nil);
...
procedure TChartOfAccountIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TChartOfAccountIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TChartOfAccountIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TChartOfAccountIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TChartOfAccountIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TChartOfAccountIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TChartOfAccountIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
