unit uBillPayReceive.Index.View;
...
type
  TBillPayReceiveIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  BillPayReceiveIndexView: TBillPayReceiveIndexView;
...
procedure TBillPayReceiveIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Contas a Pagar/Receber';
  pnlAppend.Width            := 0;
...
procedure TBillPayReceiveIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TBillPayReceiveIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TBillPayReceiveShowDTO> = TBillPayReceiveInputView.Handle(TEntityState.Store);
...
procedure TBillPayReceiveIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TBillPayReceiveService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TBillPayReceiveIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TBillPayReceiveShowDTO> = TBillPayReceiveInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TBillPayReceiveIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TBillPayReceiveIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TBillPayReceiveIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TBillPayReceiveIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TBillPayReceiveIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TBillPayReceiveInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TBillPayReceiveIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TBillPayReceiveIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'bill_pay_receive.batch';
...
procedure TBillPayReceiveIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TBillPayReceiveIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TBillPayReceiveIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TBillPayReceiveIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'bill_pay_receive.'+LSelectedColumn;
...
procedure TBillPayReceiveIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TBillPayReceiveFilterDTO> = TBillPayReceiveFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TBillPayReceiveService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TBillPayReceiveIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TBillPayReceiveIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TBillPayReceiveIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  cbxFilterIndex.Clear;
  cbxFilterIndex.Items.Add('ID ou Lote');
...
procedure TBillPayReceiveIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TBillPayReceiveIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TBillPayReceiveIndexView> = TBillPayReceiveIndexView.Create(nil);
...
procedure TBillPayReceiveIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TBillPayReceiveIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TBillPayReceiveIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TBillPayReceiveIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TBillPayReceiveIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TBillPayReceiveIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TBillPayReceiveIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
