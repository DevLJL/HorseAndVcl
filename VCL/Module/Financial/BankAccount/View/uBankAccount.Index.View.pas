...
type
  TBankAccountIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  BankAccountIndexView: TBankAccountIndexView;
...
procedure TBankAccountIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Conta Bancárias';
  pnlAppend.Width            := 0;
...
procedure TBankAccountIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TBankAccountIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TBankAccountShowDTO> = TBankAccountInputView.Handle(TEntityState.Store);
...
procedure TBankAccountIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TBankAccountService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TBankAccountIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TBankAccountShowDTO> = TBankAccountInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TBankAccountIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TBankAccountIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TBankAccountIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TBankAccountIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TBankAccountIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TBankAccountInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TBankAccountIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TBankAccountIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'bank_account.name';
...
procedure TBankAccountIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TBankAccountIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TBankAccountIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TBankAccountIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'bank_account.'+LSelectedColumn;
...
procedure TBankAccountIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TBankAccountFilterDTO> = TBankAccountFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TBankAccountService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TBankAccountIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TBankAccountIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TBankAccountIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TBankAccountIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TBankAccountIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TBankAccountIndexView> = TBankAccountIndexView.Create(nil);
...
procedure TBankAccountIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TBankAccountIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TBankAccountIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TBankAccountIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TBankAccountIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TBankAccountIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TBankAccountIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
