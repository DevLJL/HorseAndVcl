...
type
  TPaymentIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  PaymentIndexView: TPaymentIndexView;
...
procedure TPaymentIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Pagamentos';
  pnlAppend.Width            := 0;
...
procedure TPaymentIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TPaymentIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TPaymentShowDTO> = TPaymentInputView.Handle(TEntityState.Store);
...
procedure TPaymentIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TPaymentService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TPaymentIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TPaymentShowDTO> = TPaymentInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TPaymentIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TPaymentIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TPaymentInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TPaymentIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TPaymentIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TPaymentIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TPaymentIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TPaymentIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'payment.'+LSelectedColumn;
...
procedure TPaymentIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TPaymentFilterDTO> = TPaymentFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TPaymentService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TPaymentIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TPaymentIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TPaymentIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TPaymentIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TPaymentIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TPaymentIndexView> = TPaymentIndexView.Create(nil);
...
procedure TPaymentIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TPaymentIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TPaymentIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TPaymentIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TPaymentIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TPaymentIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
