...
type
  TCostCenterIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  CostCenterIndexView: TCostCenterIndexView;
...
procedure TCostCenterIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Centros de Custo';
  pnlAppend.Width            := 0;
...
procedure TCostCenterIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TCostCenterIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TCostCenterShowDTO> = TCostCenterInputView.Handle(TEntityState.Store);
...
procedure TCostCenterIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TCostCenterService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TCostCenterIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TCostCenterShowDTO> = TCostCenterInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TCostCenterIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TCostCenterIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TCostCenterIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TCostCenterIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TCostCenterIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TCostCenterInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TCostCenterIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TCostCenterIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'cost_center.name';
...
procedure TCostCenterIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TCostCenterIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TCostCenterIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TCostCenterIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'cost_center.'+LSelectedColumn;
...
procedure TCostCenterIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TCostCenterFilterDTO> = TCostCenterFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TCostCenterService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TCostCenterIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TCostCenterIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TCostCenterIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TCostCenterIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TCostCenterIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TCostCenterIndexView> = TCostCenterIndexView.Create(nil);
...
procedure TCostCenterIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TCostCenterIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TCostCenterIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TCostCenterIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TCostCenterIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
