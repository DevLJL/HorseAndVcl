...
type
  TAdditionalIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  AdditionalIndexView: TAdditionalIndexView;
...
procedure TAdditionalIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Adicionals';
  pnlAppend.Width            := 0;
...
procedure TAdditionalIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TAdditionalIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TAdditionalShowDTO> = TAdditionalInputView.Handle(TEntityState.Store);
...
procedure TAdditionalIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TAdditionalService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TAdditionalIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TAdditionalShowDTO> = TAdditionalInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TAdditionalIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TAdditionalIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TAdditionalIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TAdditionalIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TAdditionalIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TAdditionalInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TAdditionalIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TAdditionalIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'additional.name';
...
procedure TAdditionalIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TAdditionalIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TAdditionalIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TAdditionalIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'additional.'+LSelectedColumn;
...
procedure TAdditionalIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TAdditionalFilterDTO> = TAdditionalFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TAdditionalService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TAdditionalIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TAdditionalIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TAdditionalIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TAdditionalIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TAdditionalIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TAdditionalIndexView> = TAdditionalIndexView.Create(nil);
...
procedure TAdditionalIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TAdditionalIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TAdditionalIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TAdditionalIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TAdditionalIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TAdditionalIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TAdditionalIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
