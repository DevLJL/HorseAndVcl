...
type
  TPriceListIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  PriceListIndexView: TPriceListIndexView;
...
procedure TPriceListIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Listas de Preços';
  pnlAppend.Width            := 0;
...
procedure TPriceListIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TPriceListIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TPriceListShowDTO> = TPriceListInputView.Handle(TEntityState.Store);
...
procedure TPriceListIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TPriceListService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TPriceListIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TPriceListShowDTO> = TPriceListInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TPriceListIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TPriceListIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TPriceListIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TPriceListIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TPriceListIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TPriceListInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TPriceListIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TPriceListIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'price_list.name';
...
procedure TPriceListIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TPriceListIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TPriceListIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TPriceListIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'price_list.'+LSelectedColumn;
...
procedure TPriceListIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TPriceListFilterDTO> = TPriceListFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TPriceListService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TPriceListIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TPriceListIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TPriceListIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TPriceListIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TPriceListIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TPriceListIndexView> = TPriceListIndexView.Create(nil);
...
procedure TPriceListIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TPriceListIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TPriceListIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TPriceListIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TPriceListIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TPriceListIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TPriceListIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
