...
type
  TCategoryIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  CategoryIndexView: TCategoryIndexView;
...
procedure TCategoryIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Categorias';
  pnlAppend.Width            := 0;
...
procedure TCategoryIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TCategoryIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TCategoryShowDTO> = TCategoryInputView.Handle(TEntityState.Store);
...
procedure TCategoryIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TCategoryService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TCategoryIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TCategoryShowDTO> = TCategoryInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TCategoryIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TCategoryIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TCategoryIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TCategoryIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TCategoryIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TCategoryInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TCategoryIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TCategoryIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TCategoryIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TCategoryIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TCategoryIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TCategoryIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'category.'+LSelectedColumn;
...
procedure TCategoryIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TCategoryFilterDTO> = TCategoryFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TCategoryService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TCategoryIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TCategoryIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TCategoryIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TCategoryIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TCategoryIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TCategoryIndexView> = TCategoryIndexView.Create(nil);
...
procedure TCategoryIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TCategoryIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TCategoryIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TCategoryIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TCategoryIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TCategoryIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TCategoryIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
