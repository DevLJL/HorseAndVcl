...
type
  TNcmIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
procedure TNcmIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... NCM';
  pnlAppend.Width            := 0;
...
procedure TNcmIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TNcmIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TNcmShowDTO> = TNcmInputView.Handle(TEntityState.Store);
...
procedure TNcmIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TNcmService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TNcmIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TNcmShowDTO> = TNcmInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TNcmIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TNcmIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TNcmIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TNcmIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TNcmIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TNcmInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TNcmIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TNcmIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TNcmIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TNcmIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TNcmIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TNcmIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'ncm.'+LSelectedColumn;
...
procedure TNcmIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TNcmFilterDTO> = TNcmFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TNcmService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TNcmIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TNcmIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TNcmIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  cbxFilterIndex.Clear;
  cbxFilterIndex.Items.Add('ID, NCM (Código) ou Descrição');
...
procedure TNcmIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TNcmIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TNcmIndexView> = TNcmIndexView.Create(nil);
...
procedure TNcmIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TNcmIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TNcmIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TNcmIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TNcmIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TNcmIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TNcmIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
