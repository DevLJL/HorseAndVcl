...
type
  TUnitIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
procedure TUnitIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Unidade de Medidas';
  pnlAppend.Width            := 0;
...
procedure TUnitIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TUnitIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TUnitShowDTO> = TUnitInputView.Handle(TEntityState.Store);
...
procedure TUnitIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TUnitService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TUnitIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TUnitShowDTO> = TUnitInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TUnitIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TUnitIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TUnitIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TUnitIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TUnitIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TUnitInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TUnitIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TUnitIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TUnitIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TUnitIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TUnitIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TUnitIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'unit.'+LSelectedColumn;
...
procedure TUnitIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TUnitFilterDTO> = TUnitFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TUnitService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TUnitIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TUnitIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TUnitIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TUnitIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TUnitIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TUnitIndexView> = TUnitIndexView.Create(nil);
...
procedure TUnitIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TUnitIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TUnitIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TUnitIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TUnitIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TUnitIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TUnitIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
