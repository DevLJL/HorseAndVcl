...
begin
  ...
  Try
    ...
    const LStored: SH<TBrandShowDTO> = TBrandInputView.Handle(TEntityState.Store);
...
begin
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TBrandShowDTO> = TBrandInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TBrandInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TBrandFilterDTO> = TBrandFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TBrandService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  const LView: SH<TBrandIndexView> = TBrandIndexView.Create(nil);
