...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Locais de Armazenamento';
  pnlAppend.Width            := 0;
...
begin
  ...
  Try
    ...
    const LStored: SH<TStorageLocationShowDTO> = TStorageLocationInputView.Handle(TEntityState.Store);
...
begin
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TStorageLocationShowDTO> = TStorageLocationInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TStorageLocationInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TStorageLocationFilterDTO> = TStorageLocationFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TStorageLocationService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  const LView: SH<TStorageLocationIndexView> = TStorageLocationIndexView.Create(nil);
