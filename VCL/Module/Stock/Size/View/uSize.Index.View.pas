...
type
  TSizeIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
procedure TSizeIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Tamanhos';
  pnlAppend.Width            := 0;
...
procedure TSizeIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TSizeIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TSizeShowDTO> = TSizeInputView.Handle(TEntityState.Store);
...
procedure TSizeIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TSizeService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TSizeIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TSizeShowDTO> = TSizeInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TSizeIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TSizeIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TSizeIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TSizeIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TSizeIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TSizeInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TSizeIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TSizeIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TSizeIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TSizeIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TSizeIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TSizeIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'size.'+LSelectedColumn;
...
procedure TSizeIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TSizeFilterDTO> = TSizeFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TSizeService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TSizeIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TSizeIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TSizeIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TSizeIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TSizeIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TSizeIndexView> = TSizeIndexView.Create(nil);
...
procedure TSizeIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TSizeIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TSizeIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TSizeIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TSizeIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TSizeIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TSizeIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
