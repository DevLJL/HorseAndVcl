...
type
  TCompanyIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  CompanyIndexView: TCompanyIndexView;
...
procedure TCompanyIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Companhias';
  pnlAppend.Width            := 0;
...
procedure TCompanyIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TCompanyIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TCompanyShowDTO> = TCompanyInputView.Handle(TEntityState.Store);
...
procedure TCompanyIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TCompanyService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TCompanyIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TCompanyShowDTO> = TCompanyInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TCompanyIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TCompanyIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TCompanyIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TCompanyIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TCompanyIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TCompanyInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TCompanyIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TCompanyIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TCompanyIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TCompanyIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TCompanyIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TCompanyIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'company.'+LSelectedColumn;
...
procedure TCompanyIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TCompanyFilterDTO> = TCompanyFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TCompanyService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TCompanyIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TCompanyIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TCompanyIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TCompanyIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TCompanyIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TCompanyIndexView> = TCompanyIndexView.Create(nil);
...
procedure TCompanyIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TCompanyIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TCompanyIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TCompanyIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TCompanyIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TCompanyIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TCompanyIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
