...
type
  TTenantIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  TenantIndexView: TTenantIndexView;
...
procedure TTenantIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
procedure TTenantIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TTenantIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TTenantShowDTO> = TTenantInputView.Handle(TEntityState.Store);
...
procedure TTenantIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TTenantService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TTenantIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TTenantShowDTO> = TTenantInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TTenantIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TTenantIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TTenantIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TTenantIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TTenantIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TTenantInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TTenantIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TTenantIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TTenantIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TTenantIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TTenantIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TTenantIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'tenant.'+LSelectedColumn;
...
procedure TTenantIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TTenantFilterDTO> = TTenantFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TTenantService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TTenantIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TTenantIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TTenantIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TTenantIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TTenantIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TTenantIndexView> = TTenantIndexView.Create(nil);
...
procedure TTenantIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TTenantIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TTenantIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TTenantIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TTenantIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TTenantIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TTenantIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
