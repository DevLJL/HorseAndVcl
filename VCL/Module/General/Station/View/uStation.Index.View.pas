...
type
  TStationIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  StationIndexView: TStationIndexView;
...
procedure TStationIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Estações';
  pnlAppend.Width            := 0;
...
procedure TStationIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TStationIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TStationShowDTO> = TStationInputView.Handle(TEntityState.Store);
...
procedure TStationIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TStationService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TStationIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TStationShowDTO> = TStationInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TStationIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TStationIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TStationIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TStationIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TStationIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TStationInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TStationIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TStationIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TStationIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TStationIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TStationIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TStationIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'station.'+LSelectedColumn;
...
procedure TStationIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TStationFilterDTO> = TStationFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TStationService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TStationIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TStationIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TStationIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TStationIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TStationIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TStationIndexView> = TStationIndexView.Create(nil);
...
procedure TStationIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TStationIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TStationIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TStationIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TStationIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TStationIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TStationIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
