...
type
  TPosPrinterIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  PosPrinterIndexView: TPosPrinterIndexView;
...
procedure TPosPrinterIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Impressoras (POS)';
  pnlAppend.Width            := 0;
...
procedure TPosPrinterIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TPosPrinterIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TPosPrinterShowDTO> = TPosPrinterInputView.Handle(TEntityState.Store);
...
procedure TPosPrinterIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TPosPrinterService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TPosPrinterIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TPosPrinterShowDTO> = TPosPrinterInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TPosPrinterIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TPosPrinterIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TPosPrinterIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TPosPrinterIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TPosPrinterIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TPosPrinterInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TPosPrinterIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TPosPrinterIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'pos_printer.name';
...
procedure TPosPrinterIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TPosPrinterIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TPosPrinterIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TPosPrinterIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'pos_printer.'+LSelectedColumn;
...
procedure TPosPrinterIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TPosPrinterFilterDTO> = TPosPrinterFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TPosPrinterService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TPosPrinterIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TPosPrinterIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TPosPrinterIndexView.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TPosPrinterIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TPosPrinterIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TPosPrinterIndexView> = TPosPrinterIndexView.Create(nil);
...
procedure TPosPrinterIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TPosPrinterIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TPosPrinterIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TPosPrinterIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TPosPrinterIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TPosPrinterIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TPosPrinterIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
