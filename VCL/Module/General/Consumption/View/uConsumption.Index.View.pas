...
type
  TConsumptionIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
...
var
  ConsumptionIndexView: TConsumptionIndexView;
...
procedure TConsumptionIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Números de Consumo';
  pnlAppend.Width            := 0;
...
procedure TConsumptionIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TConsumptionIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TConsumptionShowDTO> = TConsumptionInputView.Handle(TEntityState.Store);
...
procedure TConsumptionIndexView.btnDeleteClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    LockControl(pnlBackground);
    if not TConsumptionService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TConsumptionIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TConsumptionShowDTO> = TConsumptionInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TConsumptionIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TConsumptionIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
...
procedure TConsumptionIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TConsumptionIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TConsumptionIndexView.btnViewClick(Sender: TObject);
begin
  // Evitar erros
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (dtsIndex.DataSet.Fields[0].AsLargeInt > 0) and (LoadingSearch = False);
  if not LKeepGoing then
  ...
  // Visualizar Registro
  TConsumptionInputView.Handle(TEntityState.View, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TConsumptionIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TConsumptionIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'consumption.number';
...
procedure TConsumptionIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
  const LKeepGoing = Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0) and (LoadingSearch = False);
  if not LKeepGoing then
...
procedure TConsumptionIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TConsumptionIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TConsumptionIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy do BackEnd
    // -------------------------------------------------------------------------
    FFilterOrderBy := 'consumption.'+LSelectedColumn;
...
procedure TConsumptionIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  // Filto de Dados
  // ---------------------------------------------------------------------------
  const LFilter: SH<TConsumptionFilterDTO> = TConsumptionFilterDTO.Create;
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TConsumptionService.Make.Index(LFilter);
    end)
  .OnException_Sync(
...
procedure TConsumptionIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TConsumptionIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TConsumptionIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  cbxFilterIndex.Clear;
  cbxFilterIndex.Items.Add('ID ou Número de Consumo');
...
procedure TConsumptionIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TConsumptionIndexView.HandleLocate: Int64;
begin
  Result := -1;

  const LView: SH<TConsumptionIndexView> = TConsumptionIndexView.Create(nil);
...
procedure TConsumptionIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TConsumptionIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TConsumptionIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TConsumptionIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TConsumptionIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TConsumptionIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TConsumptionIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
