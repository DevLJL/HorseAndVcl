unit uCustomerForDelivery.Index.View;
...
type
  TCustomerForDeliveryIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
    ...
    pnlWriteToSearch: TPanel;
    imgWriteToSearch: TSkAnimatedImage;
    lblWriteToSearch: TLabel;
...
var
  CustomerForDeliveryIndexView: TCustomerForDeliveryIndexView;
...
procedure TCustomerForDeliveryIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
procedure TCustomerForDeliveryIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TCustomerForDeliveryIndexView.btnAppendClick(Sender: TObject);
begin
  inherited;
  ...
  Try
    ...
    const LStored: SH<TPersonShowDTO> = TPersonInputView.Handle(TEntityState.Store, 0, FPhone1);
...
procedure TCustomerForDeliveryIndexView.btnEditClick(Sender: TObject);
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    const LUpdated: SH<TPersonShowDTO> = TPersonInputView.Handle(TEntityState.Update, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TCustomerForDeliveryIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TCustomerForDeliveryIndexView.btnLocateConfirmClick(Sender: TObject);
begin
  inherited;
  const LKeepGoing = FLayoutLocate and Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (LoadingSearch = False);
  if not LKeepGoing then
    Exit;

  case (dtsIndex.DataSet.RecordCount > 0) of
    True: Begin
      FLocateResult := dtsIndex.DataSet.Fields[0].AsLargeInt;
      ModalResult   := mrOK;
    End;
    False: Begin
      FPhone1 := edtSearchValue.Text;
      btnAppendClick(btnAppend);
    End;
  end;
end;

procedure TCustomerForDeliveryIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TCustomerForDeliveryIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TCustomerForDeliveryIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TCustomerForDeliveryIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  const LMustSearchAnyPart = True; //(Ord(UserLogged.Current.acl_role.general_search_method) = 1);
  ...
  // Indexadores
  cbxFilterIndex.ItemIndex := 11; // Telefone 1
...
procedure TCustomerForDeliveryIndexView.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
...
procedure TCustomerForDeliveryIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TCustomerForDeliveryIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TCustomerForDeliveryIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
procedure TCustomerForDeliveryIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
  LSearchValue: String;
...
begin
  ...
  begin
    ...
    case cbxFilterIndex.ItemIndex of
      ...
      2:  name                  := LSearchValue;
      3:  alias_name            := LSearchValue;
      4:  legal_entity_number   := LSearchValue;
      5:  zipcode               := LSearchValue;
      6:  address               := LSearchValue;
      7:  address_number        := LSearchValue;
      8:  district              := LSearchValue;
      9:  city_name             := LSearchValue;
      10: reference_point       := LSearchValue;
      11: phone_1               := LSearchValue;
      12: company_email         := LSearchValue;
      13: financial_email       := LSearchValue;
    end;
  end;
  // ---------------------------------------------------------------------------
  ...
  TRunTask.Execute(
    ...
    begin
      LIndexResult := TPersonService.Make.Index(LFilter);
    end)
  .OnException_Sync(
  ...
  .OnTerminated_Sync(
    ...
    begin
      Try
        ...
        // Encerrar Loading
        pnlWriteToSearch.Visible := False;
        imgWriteToSearch.Enabled := False;
...
procedure TCustomerForDeliveryIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TCustomerForDeliveryIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TCustomerForDeliveryIndexView.FormCreate(Sender: TObject);
begin
  inherited;
  pnlWriteToSearch.BringToFront;

  // Colunas de Pesquisa da Tabela
  cbxFilterIndex.Clear;
  cbxFilterIndex.Items.Add('ID, Razão, Fantasia, Nome ou Apelido');
  cbxFilterIndex.Items.Add('ID');
  cbxFilterIndex.Items.Add('Razão / Nome');
  cbxFilterIndex.Items.Add('Fantasia / Apelido');
  cbxFilterIndex.Items.Add('CPF / CNPJ');
  cbxFilterIndex.Items.Add('Cep');
  cbxFilterIndex.Items.Add('Logradouro');
  cbxFilterIndex.Items.Add('Número');
  cbxFilterIndex.Items.Add('Bairro');
  cbxFilterIndex.Items.Add('Cidade');
  cbxFilterIndex.Items.Add('Ponto de Referência');
  cbxFilterIndex.Items.Add('Telefone 1');
  cbxFilterIndex.Items.Add('E-mail');
  cbxFilterIndex.Items.Add('E-mail Financeiro');
...
procedure TCustomerForDeliveryIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TCustomerForDeliveryIndexView.HandleLocate: Int64;
begin
  const LView: SH<TCustomerForDeliveryIndexView> = TCustomerForDeliveryIndexView.Create(nil);
...
procedure TCustomerForDeliveryIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TCustomerForDeliveryIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TCustomerForDeliveryIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TCustomerForDeliveryIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TCustomerForDeliveryIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TCustomerForDeliveryIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TCustomerForDeliveryIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
